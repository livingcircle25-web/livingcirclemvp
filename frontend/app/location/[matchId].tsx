...
const AMENITY_CFG: Record<string, { emoji: string; label: string; color: string }> = {
  metro:       { emoji: "🚇", label: "Metro",       color: "#8B5CF6" },
  ...
  atm:         { emoji: "💰", label: "ATM",         color: "#14B8A6" },
...
function buildMapHtml(data: LocationData, activeAmenity: Amenity | null): string {
  const amenityMarkersJs = data.amenities.slice(0, 24).map((a) => {
    const cfg = AMENITY_CFG[a.type] ?? { emoji: "📍", color: "#14B8A6" };
  ...
  const routeLine = data.my_lat && data.my_lng
    ? `L.polyline([[${data.my_lat},${data.my_lng}],[${data.match_lat},${data.match_lng}]],{color:'#8B5CF6',weight:2.5,opacity:0.85,dashArray:'9,6'}).addTo(map);`
    : "";

  const activeRouteLine = activeAmenity
    ? `L.polyline([[${data.match_lat},${data.match_lng}],[${activeAmenity.lat},${activeAmenity.lng}]],{color:'#14B8A6',weight:2.5,opacity:0.9,dashArray:'6,4'}).addTo(map);`
    : "";
  ...
  const myMarker = data.my_lat && data.my_lng
    ? `L.marker([${data.my_lat},${data.my_lng}],{icon:L.divIcon({html:'<div style="width:42px;height:42px;border-radius:50%;background:#8B5CF6;display:flex;align-items:center;justify-content:center;font-size:20px;box-shadow:0 0 12px rgba(139,92,246,0.55);border:2px solid #fff;">🏠</div>',iconSize:[42,42],iconAnchor:[21,21],className:''})}).addTo(map).bindPopup('<b>Your Area</b><br>${(data.my_locality ?? "").replace(/'/g, "\\'")}');`
    : "";
...
<style>
*{margin:0;padding:0;box-sizing:border-box}body{background:#F5F5FC}
#map{width:100%;height:100vh}
.leaflet-control-attribution{display:none!important}
.leaflet-popup-content-wrapper{background:#FFFFFF;color:#14163A;border:1px solid rgba(20,184,166,.35);border-radius:14px;box-shadow:0 2px 14px rgba(20,22,58,.12)}
.leaflet-popup-tip{background:#FFFFFF}
.leaflet-popup-content{color:#5B5F82;font-family:system-ui,sans-serif;font-size:13px;line-height:1.5}
.leaflet-popup-content b{color:#14B8A6;font-size:14px}
...
L.marker([${data.match_lat},${data.match_lng}],{icon:L.divIcon({
  html:'<div style="width:46px;height:46px;border-radius:50%;background:#14B8A6;display:flex;align-items:center;justify-content:center;font-size:22px;box-shadow:0 2px 12px rgba(20,184,166,0.55);border:2px solid #fff;">📍</div>',
  iconSize:[46,46],iconAnchor:[23,23],className:''
})}).addTo(map).bindPopup('<b>${data.match_name.replace(/'/g, "\\'")}</b><br>${(data.match_locality ?? "").replace(/'/g, "\\'")}').openPopup();
${myMarker}
${workMarker}
${routeLine}
${activeRouteLine}
    ...
    <View style={[styles.distCard, { borderColor: color + "70", shadowColor: color }]}>
      ...
      <View style={{ flex: 1, gap: 6 }}>
        ...
        <View style={{ flexDirection: "row", gap: 6, flexWrap: "wrap" }}>
          {driveMin   != null && <ModePill emoji="🚗" label="Drive"   min={driveMin}   color="#14B8A6" />}
          {transitMin != null && <ModePill emoji="🚌" label="Transit" min={transitMin} color="#8B5CF6" />}
        ...
        <View style={styles.amenityExpanded}>
          ...
          <View style={styles.modeGrid}>
            <View style={styles.modeCard}>
              ...
              <Text style={[styles.modeCardMin, { color: "#14B8A6" }]}>{item.drive_min} min</Text>
            </View>
            <View style={styles.modeCard}>
              <Text style={styles.modeCardEmoji}>🚌</Text>
              <Text style={styles.modeCardLabel}>Transit</Text>
              <Text style={[styles.modeCardMin, { color: "#8B5CF6" }]}>{item.transit_min} min</Text>
            </View>
            <View style={styles.modeCard}>
              <Text style={styles.modeCardEmoji}>🚶</Text>
    ...
    <View style={styles.root}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {data.match_locality && <Text style={styles.headerSub}>{data.match_locality}</Text>}
        </View>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} stickyHeaderIndices={[2]}>
        ...
        <View style={{ paddingHorizontal: 16, paddingBottom: 56 }}>
          ...
          <Text style={styles.disclaimer}>
            Map © OpenStreetMap · CartoDB · Distances approx · Times estimated for city traffic conditions
          </Text>
        </View>
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  requestBtn: {
    ...
    borderWidth: 1.5, borderColor: C.coral, backgroundColor: "rgba(24,144,255,0.08)",
    shadowColor: C.coral, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.18, shadowRadius: 8, elevation: 6,
  },
  requestBtnDone: { borderColor: C.success, backgroundColor: "rgba(82,196,26,0.10)", shadowColor: C.success },
  ...
  distCard: {
    ...
    shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.14, shadowRadius: 12, elevation: 6,
  },
  ...
  distBadge: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: R.md, borderWidth: 1, backgroundColor: "rgba(0,0,0,0.03)" },
  distBadgeText: { fontSize: 13, fontWeight: "800" },

  // Expanded detail
  amenityExpanded: { padding: 14, borderTopWidth: 1, borderTopColor: C.border, backgroundColor: "rgba(0,0,0,0.02)", gap: 10 },
  farWarning: { backgroundColor: "rgba(250,173,20,0.12)", borderWidth: 1, borderColor: "rgba(250,173,20,0.5)", borderRadius: R.md, padding: 10 },
  farWarningText: { color: "#F5A623", fontSize: 12, fontWeight: "600" },
