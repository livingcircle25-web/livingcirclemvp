...
# DeepFace (and some of its deps) log emoji characters; Windows' default console
# codepage (cp1252) can't encode them and raises UnicodeEncodeError, which would
# otherwise crash whatever request triggered that log line. Force UTF-8 stdio.
...
class Profile(BaseModel):
    ...
    verification_photo: Optional[str] = None  # base64 — PRIVATE, never returned by public_profile()
    verification_photo_embedding: Optional[List[float]] = None  # DeepFace embedding, PRIVATE
...
class UploadPhotoIn(BaseModel):
    photo_base64: str  # data URI — PRIVATE, mandatory before onboarding completes


class VerifySubmitIn(BaseModel):
    match_id: str
    media_base64: str
    media_type: str = "video"  # "video" | "photo" (fallback when video capture unavailable)


class HouseTourRequestIn(BaseModel):
    match_id: str


class HouseTourRespondIn(BaseModel):
    accept: bool


class HouseTourPhotoIn(BaseModel):
    photo_base64: str


class PoolingListingIn(BaseModel):
    address: str
    area: str
    latitude: float
    longitude: float
    bhk_type: str
    rent: int
    roommates_needed: int
    photos: List[str]  # base64 — min 3 enforced server-side
    description: Optional[str] = None
    move_in_date: str
    amenities: List[str] = []


class PoolingListingUpdate(BaseModel):
...
class PoolingRequestIn(BaseModel):
    listing_id: str


class PoolingRespondIn(BaseModel):
    accept: bool


class PoolingVerifySubmitIn(BaseModel):
    media_base64: str


class PoolingMessageIn(BaseModel):
    text: str


class PoolingReportIn(BaseModel):
    reason: str
    details: Optional[str] = None
...
def public_profile(p: dict, viewer: Optional[dict] = None, hide_photo: bool = False) -> dict:
    out = {
        "user_id": p["user_id"],
        "name": p.get("name"),
        "age": p.get("age"),
        "gender": p.get("gender"),
        "photo": None if hide_photo else p.get("photo"),
        "bio": p.get("bio"),
...
@api.post("/profile/upload-photo")
async def upload_verification_photo(body: UploadPhotoIn, authorization: Optional[str] = Header(None)):
    """
    Mandatory verification photo, captured during onboarding.
    Stored server-side only — intentionally excluded from public_profile()'s
    field whitelist, so no API ever returns it to another user.

    A face embedding (DeepFace) is extracted up front so later match-time
    verification photos can be compared against it — this also doubles as
    an onboarding-time quality gate (rejects photos with no detectable face).
    """
    u = await get_user(authorization)
    if len(body.photo_base64) > 8_000_000:
        raise HTTPException(413, "Image too large. Max ~5 MB.")
    embedding, error = await asyncio.to_thread(face_service.extract_face_embedding, body.photo_base64)
    if error:
        raise HTTPException(400, "❌ Photo not clear. Please ensure: face is clearly visible, good lighting, front-facing, no filters. Try again.")
    await db.profiles.update_one(
        {"user_id": u["user_id"]},
        {"$set": {"verification_photo": body.photo_base64, "verification_photo_embedding": embedding}},
    )
    return {"ok": True}
...
async def discover(
    ...
    # Photos are hidden pre-match (privacy) — same "reveal after match" pattern
    # already used for room/listing photos elsewhere in the app.
    out = [public_profile(d, u, hide_photo=True) for d in docs]
...
def _verify_keys(m: dict, user_id: str) -> tuple[str, str]:
    """Returns (my_verified_key, other_verified_key) — users[0] is 'a', users[1] is 'b'."""
    key = "user_a_verified" if m["users"][0] == user_id else "user_b_verified"
    other = "user_b_verified" if key == "user_a_verified" else "user_a_verified"
    return key, other
        ...
        if reverse:
            ...
            if not existing:
                await db.matches.insert_one({
                    "match_id": match_id, "users": key, "created_at": now_iso(),
                    "user_a_verified": False, "user_b_verified": False, "can_chat": False,
                })
...
async def matches(authorization: Optional[str] = Header(None)):
    ...
    for m in docs:
        ...
        my_key, other_key = _verify_keys(m, u["user_id"])
        out.append({"match_id": m["match_id"], "user": public_profile(other, u),
                    "last_message": last, "created_at": m["created_at"],
                    "can_chat": bool(m.get("can_chat", False)),
                    "my_verified": bool(m.get(my_key, False)),
                    "other_verified": bool(m.get(other_key, False))})
...
async def match_with_bot(authorization: Optional[str] = Header(None)):
    ...
    # Upsert match so button is idempotent. Bot auto-verifies instantly so
    # testers only have to complete their own side of the verification flow.
    existing = await db.matches.find_one({"match_id": match_id}, {"_id": 0})
    if not existing:
        bot_key, human_key = ("user_a_verified", "user_b_verified") if key[0] == BOT_ID else ("user_b_verified", "user_a_verified")
        await db.matches.insert_one({
            "match_id": match_id, "users": key, "created_at": now_iso(),
            bot_key: True, human_key: False, "can_chat": False,
        })
...
async def _run_face_verification(u: dict, submitted_media_base64: str) -> dict:
    """
    Shared face-match pipeline for both 1:1 match verification and pooling
    verification. Runs blocking DeepFace/OpenCV work off the event loop via
    asyncio.to_thread. Gender is informational only — never a rejection reason
    (see services/face_recognition.py docstring for why).
    """
    presence = await asyncio.to_thread(face_service.check_face_presence, submitted_media_base64)
    if not presence["face_detected"]:
        return {"status": "rejected", "similarity": 0.0, "message": f"❌ {presence['message']}"}

    # Real anti-spoofing (DeepFace anti_spoofing=True) — only meaningful because
    # the frontend captures this frame live from getUserMedia/the device camera
    # with no upload option, so the model is actually looking at what the
    # camera saw, not an attacker-controlled file.
    liveness = await asyncio.to_thread(face_service.check_liveness, submitted_media_base64)
    if not liveness["is_live"]:
        return {"status": "rejected", "similarity": 0.0, "message": f"❌ {liveness['message']}", "liveness_score": liveness["score"]}

    gender = await asyncio.to_thread(face_service.estimate_gender, submitted_media_base64, u.get("gender"))
    gender_note = None
    if gender["matches_profile"] is False:
        gender_note = f"Detected gender ({gender['detected_gender']}) didn't match your profile — informational only, did not affect your result."

    compare = await asyncio.to_thread(face_service.compare_faces, u["verification_photo"], submitted_media_base64)
    compare = {**compare, "liveness_score": liveness["score"]}
    if gender_note:
        compare["gender_note"] = gender_note
    return compare


# ---------- Match Verification (real DeepFace face-match against the private
# onboarding verification photo — see services/face_recognition.py for the
# honesty notes on what the "liveness"/gender signals actually measure) ----------
@api.post("/verification/submit")
async def submit_verification(body: VerifySubmitIn, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    m = await db.matches.find_one({"match_id": body.match_id}, {"_id": 0})
    if not m or u["user_id"] not in m["users"]:
        raise HTTPException(404, "Match not found")
    if body.media_type not in ("video", "photo"):
        raise HTTPException(400, "Invalid media_type")
    if len(body.media_base64) > 15_000_000:
        raise HTTPException(413, "File too large. Max ~10 MB.")
    if not u.get("verification_photo"):
        raise HTTPException(400, "Complete your onboarding verification photo first.")

    face_result = await _run_face_verification(u, body.media_base64)
    ...
    await db.verifications.insert_one({
        ...
        "gender_note": face_result.get("gender_note"),
        "created_at": now_iso(),
    })

    approved = face_result["status"] == "approved"
    my_key, other_key = _verify_keys(m, u["user_id"])
    other_verified = bool(m.get(other_key, False))
    can_chat = False
    if approved:
        can_chat = other_verified
        await db.matches.update_one(
            {"match_id": body.match_id},
            {"$set": {my_key: True, "can_chat": can_chat}},
        )
    return {
        "ok": True, "status": face_result["status"], "similarity": face_result["similarity"],
        "message": face_result["message"], "my_verified": approved,
        "other_verified": other_verified, "can_chat": can_chat,
    }


@api.get("/verification/status")
async def verification_status(match_id: str, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    m = await db.matches.find_one({"match_id": match_id}, {"_id": 0})
    if not m or u["user_id"] not in m["users"]:
        raise HTTPException(404, "Match not found")
...
# ---------- House Tour (async in-app photo capture, not live video) ----------
@api.post("/house-tour/request")
async def house_tour_request(body: HouseTourRequestIn, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    m = await db.matches.find_one({"match_id": body.match_id}, {"_id": 0})
    if not m or u["user_id"] not in m["users"]:
        raise HTTPException(404, "Match not found")
    if not m.get("can_chat", False):
        raise HTTPException(403, "Complete verification before requesting a house tour")
    receiver_id = next(uid for uid in m["users"] if uid != u["user_id"])
    tour = {
        "id": str(uuid.uuid4()),
        "match_id": body.match_id,
    ...
    await db.house_tours.insert_one(tour)
    await db.messages.insert_one({
        "id": str(uuid.uuid4()), "match_id": body.match_id, "sender_id": u["user_id"],
        "text": f"📷 {u.get('name', 'Your match')} requested a house tour.",
        "is_system": True, "at": now_iso(),
    })
    tour.pop("_id", None)
    return tour


@api.post("/house-tour/{tour_id}/respond")
async def house_tour_respond(tour_id: str, body: HouseTourRespondIn, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    t = await db.house_tours.find_one({"id": tour_id}, {"_id": 0})
    if not t or u["user_id"] != t["receiver_id"]:
        raise HTTPException(404, "Tour request not found")
    if t["status"] != "requested":
        raise HTTPException(400, "This request was already responded to")
    new_status = "accepted" if body.accept else "declined"
    await db.house_tours.update_one(
        {"id": tour_id},
        {"$set": {"status": new_status, "accepted_at": now_iso() if body.accept else None}},
    )
    await db.messages.insert_one({
        "id": str(uuid.uuid4()), "match_id": t["match_id"], "sender_id": u["user_id"],
        "text": f"📷 House tour request {'accepted ✅' if body.accept else 'declined'}.",
        "is_system": True, "at": now_iso(),
    })
    return {"ok": True, "status": new_status}


@api.post("/house-tour/{tour_id}/photo")
async def house_tour_add_photo(tour_id: str, body: HouseTourPhotoIn, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    t = await db.house_tours.find_one({"id": tour_id}, {"_id": 0})
    if not t or u["user_id"] not in (t["requester_id"], t["receiver_id"]):
        raise HTTPException(404, "Tour not found")
    if t["status"] != "accepted":
        raise HTTPException(400, "Tour is not active")
    if len(body.photo_base64) > 8_000_000:
        raise HTTPException(413, "Image too large. Max ~5 MB.")
    photo = {"photo_id": str(uuid.uuid4()), "encrypted_data": body.photo_base64,
             "by": u["user_id"], "at": now_iso()}
    await db.house_tours.update_one({"id": tour_id}, {"$push": {"photos": photo}})
    return photo


@api.delete("/house-tour/{tour_id}/photo/{photo_id}")
async def house_tour_delete_photo(tour_id: str, photo_id: str, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    t = await db.house_tours.find_one({"id": tour_id}, {"_id": 0})
    if not t or u["user_id"] not in (t["requester_id"], t["receiver_id"]):
        raise HTTPException(404, "Tour not found")
    await db.house_tours.update_one({"id": tour_id}, {"$pull": {"photos": {"photo_id": photo_id}}})
    return {"ok": True}


@api.post("/house-tour/{tour_id}/complete")
async def house_tour_complete(tour_id: str, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    t = await db.house_tours.find_one({"id": tour_id}, {"_id": 0})
    if not t or u["user_id"] not in (t["requester_id"], t["receiver_id"]):
        raise HTTPException(404, "Tour not found")
    await db.house_tours.update_one({"id": tour_id}, {"$set": {"status": "completed", "ended_at": now_iso()}})
    return {"ok": True}


@api.get("/house-tour/match/{match_id}")
async def house_tour_for_match(match_id: str, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    m = await db.matches.find_one({"match_id": match_id}, {"_id": 0})
    if not m or u["user_id"] not in m["users"]:
        raise HTTPException(404, "Match not found")
    tours = await db.house_tours.find({"match_id": match_id}, {"_id": 0}).sort("created_at", -1).to_list(20)
    return tours


# ---------- House Pooling ----------
PII_RE = re.compile(r'(\+?\d[\d\-\s]{8,14}\d)|([^\s@]+@[^\s@]+\.[^\s@]+)')


def public_pooling_listing(l: dict, owner: Optional[dict] = None) -> dict:
    out = {k: l.get(k) for k in (
        "id", "owner_id", "address", "area", "latitude", "longitude", "bhk_type", "rent",
        "roommates_needed", "roommates_current", "photos", "description", "move_in_date",
        "amenities", "status", "created_at",
    )}
    if owner:
        out["owner"] = {
            "user_id": owner["user_id"], "name": owner.get("name"), "age": owner.get("age"),
            "occupation": owner.get("occupation"), "org": owner.get("org"),
            "gender": owner.get("gender"), "photo": owner.get("photo"),
            "id_verified": bool(owner.get("id_verified")),
            "lifestyle": owner.get("lifestyle", {}),
            "bio": owner.get("bio"),
        }
    return out


@api.post("/pooling/listings")
async def create_pooling_listing(body: PoolingListingIn, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    if not u.get("id_verified"):
        raise HTTPException(403, "Complete ID verification before posting a listing")
    if len(body.photos) < 3:
        raise HTTPException(400, "At least 3 photos required")
    for p in body.photos:
        if len(p) > 8_000_000:
            raise HTTPException(413, "One or more photos too large. Max ~5 MB each.")
    listing_id = str(uuid.uuid4())
    doc = {
        "id": listing_id,
        "owner_id": u["user_id"],
        "address": body.address.strip(), "area": body.area,
        "latitude": body.latitude, "longitude": body.longitude,
        "bhk_type": body.bhk_type, "rent": body.rent,
        "roommates_needed": body.roommates_needed, "roommates_current": 0,
        "photos": [{"photo_id": str(uuid.uuid4()), "encrypted_data": p} for p in body.photos],
    ...
    await db.house_pooling_listings.insert_one(doc)
    doc.pop("_id", None)
    return doc


@api.get("/pooling/my-listings")
async def my_pooling_listings(authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    docs = await db.house_pooling_listings.find(
        {"owner_id": u["user_id"], "status": {"$ne": "deleted"}}, {"_id": 0}
    ).sort("created_at", -1).to_list(100)
    return docs


@api.put("/pooling/listings/{listing_id}")
async def edit_pooling_listing(listing_id: str, body: PoolingListingUpdate, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    l = await db.house_pooling_listings.find_one({"id": listing_id}, {"_id": 0})
    if not l or l["owner_id"] != u["user_id"]:
        raise HTTPException(404, "Listing not found")
    updates = {k: v for k, v in body.model_dump(exclude_unset=True).items() if v is not None}
    if "photos" in updates:
        updates["photos"] = [{"photo_id": str(uuid.uuid4()), "encrypted_data": p} for p in updates["photos"]]
    updates["updated_at"] = now_iso()
    await db.house_pooling_listings.update_one({"id": listing_id}, {"$set": updates})
    out = await db.house_pooling_listings.find_one({"id": listing_id}, {"_id": 0})
    return out


@api.delete("/pooling/listings/{listing_id}")
async def delete_pooling_listing(listing_id: str, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    l = await db.house_pooling_listings.find_one({"id": listing_id}, {"_id": 0})
    if not l or l["owner_id"] != u["user_id"]:
        raise HTTPException(404, "Listing not found")
    await db.house_pooling_listings.update_one({"id": listing_id}, {"$set": {"status": "deleted", "updated_at": now_iso()}})
    return {"ok": True}


@api.get("/pooling/listings")
async def browse_pooling_listings(
    authorization: Optional[str] = Header(None),
    area: Optional[str] = None,
    ...
    u = await get_user(authorization)
    q: dict = {"status": "open", "owner_id": {"$ne": u["user_id"]}}
    if area:
        q["area"] = area
    if bhk_type:
        q["bhk_type"] = bhk_type
    docs = await db.house_pooling_listings.find(q, {"_id": 0}).sort("created_at", -1).to_list(200)
    if rent_min is not None or rent_max is not None:
        rmn = rent_min if rent_min is not None else 0
        rmx = rent_max if rent_max is not None else 10**9
        docs = [d for d in docs if rmn <= d["rent"] <= rmx]
    docs = [d for d in docs if d.get("roommates_current", 0) < d.get("roommates_needed", 0)]
    out = []
    for d in docs:
        owner = await db.profiles.find_one({"user_id": d["owner_id"]}, {"_id": 0})
        out.append(public_pooling_listing(d, owner))
    return out


@api.get("/pooling/listings/{listing_id}")
async def get_pooling_listing(listing_id: str, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    l = await db.house_pooling_listings.find_one({"id": listing_id}, {"_id": 0})
    if not l or l["status"] == "deleted":
        raise HTTPException(404, "Listing not found")
    owner = await db.profiles.find_one({"user_id": l["owner_id"]}, {"_id": 0})
    out = public_pooling_listing(l, owner)
    grp = await db.pooling_groups.find_one({"listing_id": listing_id}, {"_id": 0})
    members = []
    if grp:
        for mid in grp.get("verified_members", []):
            if mid == l["owner_id"]:
                continue
            mp = await db.profiles.find_one({"user_id": mid}, {"_id": 0})
            if mp:
                members.append({"user_id": mid, "name": mp.get("name"), "age": mp.get("age"), "occupation": mp.get("occupation")})
    out["joined_members"] = members
    my_req = await db.pooling_requests.find_one({"listing_id": listing_id, "requester_id": u["user_id"]}, {"_id": 0})
    out["my_request_status"] = my_req["status"] if my_req else None
    out["is_owner"] = l["owner_id"] == u["user_id"]
    return out


@api.post("/pooling/requests")
async def send_pooling_request(body: PoolingRequestIn, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    l = await db.house_pooling_listings.find_one({"id": body.listing_id}, {"_id": 0})
    if not l or l["status"] != "open":
        raise HTTPException(404, "Listing not found")
    if l["owner_id"] == u["user_id"]:
        raise HTTPException(400, "Cannot request your own listing")
    existing = await db.pooling_requests.find_one({"listing_id": body.listing_id, "requester_id": u["user_id"]}, {"_id": 0})
    if existing and existing["status"] == "pending":
        raise HTTPException(400, "Request already sent")
    req_id = existing["id"] if existing else str(uuid.uuid4())
    doc = {
        "id": req_id, "listing_id": body.listing_id,
        "requester_id": u["user_id"], "owner_id": l["owner_id"],
        "status": "pending", "verified": False,
        "created_at": now_iso(), "response_at": None,
    }
    await db.pooling_requests.update_one(
        {"listing_id": body.listing_id, "requester_id": u["user_id"]},
        {"$set": doc}, upsert=True,
    )
    return {"ok": True, "request_id": req_id}


@api.get("/pooling/my-requests")
async def my_pooling_requests(authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    docs = await db.pooling_requests.find({"requester_id": u["user_id"]}, {"_id": 0}).sort("created_at", -1).to_list(200)
    out = []
    for r in docs:
        l = await db.house_pooling_listings.find_one({"id": r["listing_id"]}, {"_id": 0})
        out.append({**r, "listing": public_pooling_listing(l) if l else None})
    return out


@api.get("/pooling/listings/{listing_id}/requests")
async def listing_requests(listing_id: str, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    l = await db.house_pooling_listings.find_one({"id": listing_id}, {"_id": 0})
    if not l or l["owner_id"] != u["user_id"]:
        raise HTTPException(404, "Listing not found")
    docs = await db.pooling_requests.find({"listing_id": listing_id, "status": "pending"}, {"_id": 0}).sort("created_at", -1).to_list(200)
    out = []
    for r in docs:
        p = await db.profiles.find_one({"user_id": r["requester_id"]}, {"_id": 0})
        out.append({**r, "requester": public_profile(p) if p else None})
    return out


@api.post("/pooling/requests/{request_id}/respond")
async def respond_pooling_request(request_id: str, body: PoolingRespondIn, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    r = await db.pooling_requests.find_one({"id": request_id}, {"_id": 0})
    if not r or r["owner_id"] != u["user_id"]:
        raise HTTPException(404, "Request not found")
    if r["status"] != "pending":
        raise HTTPException(400, "Already responded")
    new_status = "accepted" if body.accept else "rejected"
    await db.pooling_requests.update_one({"id": request_id}, {"$set": {"status": new_status, "response_at": now_iso()}})
    if body.accept:
        grp = await db.pooling_groups.find_one({"listing_id": r["listing_id"]}, {"_id": 0})
        if not grp:
            await db.pooling_groups.insert_one({
                "id": str(uuid.uuid4()), "listing_id": r["listing_id"], "owner_id": u["user_id"],
                "members": [u["user_id"]], "verified_members": [u["user_id"]],
                "created_at": now_iso(),
            })
        await db.pooling_groups.update_one(
            {"listing_id": r["listing_id"]},
            {"$addToSet": {"members": r["requester_id"]}},
        )
    return {"ok": True, "status": new_status}


@api.post("/pooling/requests/{request_id}/verify")
async def submit_pooling_verification(request_id: str, body: PoolingVerifySubmitIn, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    r = await db.pooling_requests.find_one({"id": request_id}, {"_id": 0})
    if not r or r["requester_id"] != u["user_id"]:
        raise HTTPException(404, "Request not found")
    if r["status"] != "accepted":
        raise HTTPException(400, "Request must be accepted before verifying")
    if len(body.media_base64) > 15_000_000:
        raise HTTPException(413, "File too large. Max ~10 MB.")
    if not u.get("verification_photo"):
        raise HTTPException(400, "Complete your onboarding verification photo first.")
    ...
    await db.pooling_verifications.insert_one({
        "id": str(uuid.uuid4()), "request_id": request_id, "user_id": u["user_id"],
        "media_base64": body.media_base64, "status": face_result["status"],
        "face_match_score": face_result["similarity"], "liveness_score": face_result.get("liveness_score"),
        "gender_note": face_result.get("gender_note"),
        "created_at": now_iso(),
    })
    if not approved:
        return {"ok": True, "verified": False, "status": face_result["status"],
                "similarity": face_result["similarity"], "message": face_result["message"]}

    await db.pooling_requests.update_one({"id": request_id}, {"$set": {"verified": True}})
    await db.pooling_groups.update_one(
        {"listing_id": r["listing_id"]},
        {"$addToSet": {"verified_members": u["user_id"]}},
    )
    await db.house_pooling_listings.update_one(
        {"id": r["listing_id"]}, {"$inc": {"roommates_current": 1}},
    )
    await db.pooling_messages.insert_one({
        "id": str(uuid.uuid4()), "group_id": r["listing_id"], "sender_id": u["user_id"],
        "text": f"✅ {u.get('name', 'A member')} joined the group.",
        "is_system": True, "contains_personal_details": False, "created_at": now_iso(),
    })
    return {"ok": True, "verified": True, "status": "approved", "similarity": face_result["similarity"], "message": face_result["message"]}


@api.get("/pooling/groups/{listing_id}")
async def get_pooling_group(listing_id: str, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    grp = await db.pooling_groups.find_one({"listing_id": listing_id}, {"_id": 0})
    if not grp or u["user_id"] not in grp["members"]:
    ...
    for mid in grp["members"]:
        ...
        if mp:
            members.append({
                "user_id": mid, "name": mp.get("name"), "age": mp.get("age"),
                "occupation": mp.get("occupation"), "photo": mp.get("photo"),
                "is_owner": mid == grp["owner_id"],
                "verified": mid in grp["verified_members"],
            })
    l = await db.house_pooling_listings.find_one({"id": listing_id}, {"_id": 0})
    return {
        "listing_id": listing_id, "address": l.get("address") if l else None,
        "members": members, "my_verified": u["user_id"] in grp["verified_members"],
        "is_owner": grp["owner_id"] == u["user_id"],
    }


@api.get("/pooling/groups/{listing_id}/messages")
async def get_pooling_messages(listing_id: str, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    grp = await db.pooling_groups.find_one({"listing_id": listing_id}, {"_id": 0})
    if not grp or u["user_id"] not in grp.get("verified_members", []):
        raise HTTPException(403, "Verification pending")
    msgs = await db.pooling_messages.find({"group_id": listing_id}, {"_id": 0}).sort("created_at", 1).to_list(1000)
    return msgs


@api.post("/pooling/groups/{listing_id}/messages")
async def send_pooling_message(listing_id: str, body: PoolingMessageIn, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    grp = await db.pooling_groups.find_one({"listing_id": listing_id}, {"_id": 0})
    if not grp or u["user_id"] not in grp.get("verified_members", []):
        raise HTTPException(403, "Verification pending")
    ...
    if not text:
        raise HTTPException(400, "Empty message")
    doc = {
        "id": str(uuid.uuid4()), "group_id": listing_id, "sender_id": u["user_id"],
        "text": text, "is_system": False,
        "contains_personal_details": bool(PII_RE.search(text)),
        "created_at": now_iso(),
    }
    await db.pooling_messages.insert_one(doc)
    doc.pop("_id", None)
    return doc


@api.post("/pooling/groups/{listing_id}/leave")
async def leave_pooling_group(listing_id: str, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    grp = await db.pooling_groups.find_one({"listing_id": listing_id}, {"_id": 0})
    if not grp or u["user_id"] not in grp["members"]:
        raise HTTPException(404, "Group not found")
    if grp["owner_id"] == u["user_id"]:
        raise HTTPException(400, "Owner cannot leave — delete the listing instead")
    await db.pooling_groups.update_one(
        {"listing_id": listing_id},
        {"$pull": {"members": u["user_id"], "verified_members": u["user_id"]}},
    )
    await db.house_pooling_listings.update_one({"id": listing_id}, {"$inc": {"roommates_current": -1}})
    await db.pooling_messages.insert_one({
        "id": str(uuid.uuid4()), "group_id": listing_id, "sender_id": u["user_id"],
        "text": f"{u.get('name', 'A member')} left the group.", "is_system": True,
        "contains_personal_details": False, "created_at": now_iso(),
    })
    return {"ok": True}


@api.post("/pooling/groups/{listing_id}/remove/{member_id}")
async def remove_pooling_member(listing_id: str, member_id: str, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    grp = await db.pooling_groups.find_one({"listing_id": listing_id}, {"_id": 0})
    if not grp or grp["owner_id"] != u["user_id"]:
        raise HTTPException(404, "Group not found")
    if member_id == u["user_id"]:
        raise HTTPException(400, "Cannot remove yourself")
    await db.pooling_groups.update_one(
        {"listing_id": listing_id},
        {"$pull": {"members": member_id, "verified_members": member_id}},
    )
    await db.house_pooling_listings.update_one({"id": listing_id}, {"$inc": {"roommates_current": -1}})
    mp = await db.profiles.find_one({"user_id": member_id}, {"_id": 0})
    await db.pooling_messages.insert_one({
        "id": str(uuid.uuid4()), "group_id": listing_id, "sender_id": u["user_id"],
        "text": f"{(mp or {}).get('name', 'A member')} was removed from the group.", "is_system": True,
        "contains_personal_details": False, "created_at": now_iso(),
    })
    return {"ok": True}


@api.post("/pooling/groups/{listing_id}/report/{member_id}")
async def report_pooling_member(listing_id: str, member_id: str, body: PoolingReportIn, authorization: Optional[str] = Header(None)):
    u = await get_user(authorization)
    grp = await db.pooling_groups.find_one({"listing_id": listing_id}, {"_id": 0})
    if not grp or u["user_id"] not in grp["members"]:
        raise HTTPException(404, "Group not found")
    ...
    doc = {
        "id": str(uuid.uuid4()), "reporter_id": u["user_id"], "reported_user_id": member_id,
        "listing_id": listing_id, "reason": body.reason, "details": (body.details or "").strip() or None,
        "status": "pending", "at": now_iso(),
    }
    await db.safety_reports.insert_one(doc)
    await db.profiles.update_one({"user_id": u["user_id"]}, {"$addToSet": {"blocked": member_id}})
...
async def startup():
    ...
    await db.verifications.create_index([("match_id", 1), ("user_id", 1)])
    await db.house_tours.create_index("id", unique=True)
    await db.house_tours.create_index("match_id")
    await db.house_pooling_listings.create_index("id", unique=True)
    await db.house_pooling_listings.create_index("owner_id")
    await db.house_pooling_listings.create_index("status")
    await db.pooling_requests.create_index("id", unique=True)
    await db.pooling_requests.create_index([("listing_id", 1), ("requester_id", 1)], unique=True)
    await db.pooling_groups.create_index("listing_id", unique=True)
    await db.pooling_messages.create_index([("group_id", 1), ("created_at", 1)])
    await db.pooling_verifications.create_index([("request_id", 1), ("user_id", 1)])
    await seed_if_empty()
